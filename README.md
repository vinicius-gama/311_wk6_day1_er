...
the shared actor_id SMALLINT key.
2. What does the blue diamond next to the "last_update" column on the "inventory" table represent?
A blue diamond indicates the column has NN set
3. How many foreign keys does the "payments" table have? How can you tell?
3. By running the SHOW CREATE TABLE statement followed by the name of the table. Or by the lines g the two tables
##### ER Diagram upload
